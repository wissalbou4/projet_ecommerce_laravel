...
@section('content')
    ...
    <div class="max-w-xl mx-auto px-4 py-10">
        <h1 class="text-3xl font-bold text-center text-gray-800 mb-8">Ajouter une Famille</h1>

        <form action="{{ route('familles.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-xl shadow space-y-6">
            @csrf

            <div>
                <label for="famille" class="block text-sm font-medium text-gray-700 mb-1">Nom de la famille</label>
                <input type="text" name="famille" id="famille" value="{{ old('famille') }}"
