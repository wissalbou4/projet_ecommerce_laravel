            ...
            <form action="{{ route('articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="codebarre" id="codebarre" value="{{ $article->codebarre }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    </div>

                    <!-- Désignation -->
                    <div>
                        <label for="designation" class="block text-sm font-medium text-gray-700">Désignation</label>
                        <input type="text" name="designation" id="designation" value="{{ $article->designation }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="prix_ht" id="prix_ht" value="{{ $article->prix_ht }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    </div>

                    <!-- TVA -->
                    <div>
                        <label for="tva" class="block text-sm font-medium text-gray-700">TVA (%)</label>
                        <input type="number" step="0.01" name="tva" id="tva" value="{{ $article->tva }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="stock" id="stock" value="{{ $article->stock }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="famille_id" id="famille_id" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                            @foreach ($familles as $famille)
                                <option value="{{ $famille->id }}" {{ $article->famille_id == $famille->id ? 'selected' : '' }}>{{ $famille->famille }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select name="marque_id" id="marque_id" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                            @foreach ($marques as $marque)
                                <option value="{{ $marque->id }}" {{ $article->marque_id == $marque->id ? 'selected' : '' }}>{{ $marque->marque }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="unite_id" id="unite_id" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                            @foreach ($unites as $unite)
                                <option value="{{ $unite->id }}" {{ $article->unite_id == $unite->id ? 'selected' : '' }}>{{ $unite->unite }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    @if($article->image)
                        <div class="mt-2">
                            <img src="{{ asset('storage/'.$article->image) }}" alt="Image actuelle" class="w-32 h-32 object-cover rounded-md">
                        </div>
                    @endif
                </div>
